...
public class PlayArea extends CardPanelContainer implements CardPanelMouseListener {
	...
	public PlayArea (JScrollPane scrollPane, GameClient gameClient) {
		super(scrollPane);
	...
	public void layout () {
		...
		for (CardPanel panel : cardPanels) {
			if (!panel.gameCard.card.isLand()) continue;
			...
			for (int i = 0, n = allLands.size(); i < n; i++) {
				...
				if (firstPanel.gameCard.card.name.equals(panel.gameCard.card.name)) {
		...
		for (Row row : rows) {
			...
			for (int stackIndex = 0, stackCount = row.size(); stackIndex < stackCount; stackIndex++) {
				...
				if (RowType.other.isType(stack.get(0).gameCard.card)) {
	...
	public void mouseLeftClicked (CardPanel panel, MouseEvent evt) {
		...
		TapCard message = new TapCard();
		message.gameCardID = panel.gameCard.getId();
	...
	private class Row extends ArrayList<Stack> {
		...
		private void addAll (List<CardPanel> cardPanels, RowType type) {
			for (CardPanel panel : cardPanels) {
				if (!type.isType(panel.gameCard.card) || panel.attachedToPanel != null) continue;
				Stack stack = new Stack();
				stack.add(panel);
				add(stack);
			}
